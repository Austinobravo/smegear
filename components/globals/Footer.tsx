  ...
  return (
    <footer className="bg-smegear-secondary text-white pt-10 pb-6 px-4">
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center justify-center">
        <div className="w-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 border-b border-white/10 pb-6 mb-6">
          {contactInfo.map((item, idx) => (
            <div
              key={idx}
              className={`flex items-center gap-3 justify-center ${item.className}`}
            >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 text-white">
        ...
        <div>
          ...
          <div className="flex rounded-md overflow-hidden border border-white/20">
            ...
            />
            <button className="bg-[#4F46E5] px-3 flex items-center justify-center text-white">
              <Send size={16} />
            </button>
          </div>
          <button className="mt-4 w-full bg-[#4F46E5] text-white text-sm py-2 rounded hover:bg-[#4338ca] transition">
            SUBSCRIBE
          </button>
        </div>
      </div>
